 ...
 *   data object will contain a
 * @param {string} data.namespace - The name of the crawl job that should be created
 * @param {object} data.data - The data that should be sent to job processor
 * 
 * @return {Promise<number>} - Returns the id of newly created job
 */
exports.dispatchCrawlJobPromise = function dispatchCrawlJobPromise(job) {

    return new Promise( (resolve, reject) => {

        const kueJob = this.dispatchCrawlJob(job, (error) => {
            
            if (error) {
                return reject(error);
            }

            return resolve(kueJob.id);
        });
